    ...
    if pencarian[:cari] != ""
      ...
      if @kategori.present?
        ...
        if @food.present?
          ...
          @menu = Menu.where(food_id: @id_food)
          if @menu.count > 0
            ...
            @restoran = Restoran.where(id: @id_restoran)
              @kg = 0
      ...
      api.push( result: "Maaf, keyword yang anda masukkan tidak boleh kosong", 'status': "failed" )
      render json: { data: api }, status: :ok
    end
  end
  private
